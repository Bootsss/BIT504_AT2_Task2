...
public class Board {
        ...
        for (int row = 0; row < GameMain.ROWS; ++row) {
            for (int col = 0; col < GameMain.COLS; ++col) {
                if (cells[row][col].getContent() == Player.EMPTY) {
                    // If there's an empty cell, the game is not over yet and it's not a draw
                    return false;
                }
            }
        }
        // If there are no empty cells, the game is over and it's a draw
        return true;
    }
	...
	public void paint(Graphics g) {
		...
		for (int row = 1; row < GameMain.ROWS; ++row) {          
			g.fillRoundRect(0, GameMain.CELL_SIZE * row - GRID_WIDTH,                
					GameMain.CANVAS_WIDTH - 1, GRID_WIDTH,                
					GRID_WIDTH, GRID_WIDTH);       
			}
		for (int col = 1; col < GameMain.COLS; ++col) {          
			g.fillRoundRect(GameMain.CELL_SIZE * col - GRID_WIDTH, 0,                
